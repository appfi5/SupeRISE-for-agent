...
当前多语言支持只影响 Owner UI 中的固定界面文案和产品提示文案。

正式要求：

- 当前支持语言固定为 `en` 与 `zh`
- 默认显示语言固定为 `en`
- Owner HTTP API、wallet tools HTTP gateway 与 MCP tools 当前不新增 `locale` 参数、header 协商或多语言返回结构
- 后端响应继续保持语言无关，依赖稳定的字段名、错误码和值对象
- Owner UI 必须基于统一文案资源本地渲染英文与中文文案
- 登录前后界面必须共享同一语言状态
- 固定枚举展示值与产品提示文案由 Owner UI 本地化
- 日期、时间、数字展示与所使用 UI 框架的内置固定文案必须跟随当前语言
- 用户自行输入的联系人名称、备注、地址、交易标识与资产符号不进入翻译流程
...
- 错误信息对用户可读，对开发者可诊断
- Owner UI 不得直接把后端返回的固定英文消息当作最终产品文案；固定错误提示应优先基于错误码本地化
