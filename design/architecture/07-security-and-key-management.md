# 07 安全、加密与密钥管理

## 1. 文档目的

本文档定义安全边界、私钥加密方案、密钥提供方式、部署侧控制权和禁止事项。它是整个系统中优先级最高的技术文档之一。

## 2. 安全边界声明

本系统是本地 MVP 信用钱包，不是高价值资产托管系统。

这意味着：

- 系统需要对私钥做正式加密管理
- 系统需要避免私钥在普通流程中暴露
- 系统不承诺抵御宿主机完全失陷
- 系统不采用 HSM、MPC、远程托管或企业级 KMS 作为本期前提

## 3. 密钥层次

本期采用双层密钥设计：

- `Wallet Private Key`
- `DEK`
- `KEK`

### 3.1 `Wallet Private Key`

- 用于推导 CKB 与 ETH 身份
- 用于完成签名和转账
- 绝不明文落盘

### 3.2 `DEK`

- 每个钱包生成一个随机 `DEK`
- 用来加密钱包私钥
- 与钱包数据一起持久化，但只能以被 `KEK` 包装后的形式保存

### 3.3 `KEK`

- 系统根密钥
- 用于包装 / 解包 `DEK`
- 不进入数据库
- 默认由部署侧提供

## 4. 加密算法

固定使用：

- `AES-256-GCM`

规则：

- `KEK` 长度固定 `32 bytes`
- `DEK` 长度固定 `32 bytes`
- 每次加密都使用独立 `iv`
- 保存 `tag`
- 若 `KEK` 来源是口令字符串，则先使用 `scrypt` 派生

## 5. 加密流程

### 5.1 创建钱包

1. 生成钱包私钥
2. 生成随机 `DEK`
3. 用 `DEK` 加密钱包私钥
4. 用 `KEK` 包装 `DEK`
5. 将私钥密文、包装后的 `DEK`、`iv`、`tag` 落库

### 5.2 使用钱包

1. 从存储层读取私钥密文与包装后的 `DEK`
2. 使用 `KEK` 解包 `DEK`
3. 使用 `DEK` 解密私钥
4. 在内存中完成签名或转账

### 5.3 导入钱包

1. 校验私钥输入
2. 重新生成 `DEK`
3. 用外部 `KEK` 包装 `DEK`
4. 替换当前钱包存储

## 6. `KEK` 提供策略

### 6.1 默认原则

- 官方只发布一个运行镜像，`quickstart` / `managed` 通过 `DEPLOYMENT_PROFILE` 区分
- 模式选择必须显式，不得根据是否挂载 secret 或文件自动推断
- 正式受控部署下，应用不负责生成正式 `KEK`
- 正式受控部署下，应用只负责读取外部提供的 `KEK`
- 但官方镜像必须额外支持 `quickstart` 零应用配置启动档位

### 6.2 支持的 provider

1. `docker-secret`
2. `file-path`
3. `env`
4. `runtime-auto-file`

优先级：

1. `WALLET_KEK_PATH`
2. `WALLET_KEK`
3. `quickstart` 模式下的 `runtime-auto-file`
4. 仅限开发模式的自动生成

### 6.3 `quickstart` 零配置模式的正式例外

为支持用户以最少应用配置完成启动，官方镜像必须提供 `quickstart` 启动档位。

在该档位下：

- 若未提供外部 `KEK`，应用允许首次启动时自动生成 `KEK`
- 自动生成的 `KEK` 必须写入运行时数据目录，而不是镜像层
- 同时必须自动生成并持久化 Owner JWT secret
- 所有自动生成的运行时 secret 都必须放在独立的本地 secret 目录

正式要求：

- `quickstart` 只允许作为本地零配置体验模式
- 不得被表述为正式受控部署模式
- 若同时提供 `managed` 专用的外部 secret 或配置，系统不得隐式切换到 `managed`
- `quickstart` 与 `managed` 共用同一镜像，切换必须通过显式设置 `DEPLOYMENT_PROFILE=managed`
- `quickstart` 必须使用显式外挂的运行时数据卷，而不是容器可写层

### 6.4 为什么默认不用 env

因为环境变量泄漏面更大：

- 启动脚本可能落盘
- 进程环境可能被诊断工具读取
- CI/CD 与运行平台更容易复制传播

所以：

- `env` 只作为兼容模式
- 非默认正式方案

## 7. 安全档位

### 7.1 `quickstart` 零配置档位

适用：

- 官方镜像本地体验
- 单机演示
- 用户按官方命令显式挂载运行时数据卷启动

要求：

- 默认链环境必须落在 `testnet` preset
- 官方 quickstart 文档统一使用 `superise-agent-wallet-data:/app/runtime-data`
- 自动生成的钱包、`KEK`、Owner JWT secret 和默认 Owner 凭证都必须写入运行时目录
- 运行时目录必须是外挂持久化存储，不能依赖容器可写层
- 首次启动时必须把 Owner 凭证文件路径打印到日志
- 首次启动时允许把初始 Owner 凭证打印到日志一次，便于用户直接从 `docker logs` 取得
- 后续启动不得重复打印同一凭证明文
- 删除容器后，只要重新挂回同一运行时数据卷，系统必须恢复原钱包与原运行时 secret
- 若运行时目录处于部分损坏或部分缺失状态，系统必须 fail-fast，不得自动补生成新状态

边界：

- 不承诺根密钥由部署侧掌握
- 不承诺抵御宿主机或 Docker 运行时完全失陷
- 不作为正式生产部署口径
- 运行时数据卷只允许挂到钱包服务容器，不得暴露给 Agent 运行容器或其他非必要容器

### 7.2 `managed` 受控部署档位

适用：

- `docker-compose`
- `systemd`
- `pm2`
- 正式可运维环境

正式默认：

- 必须显式设置 `DEPLOYMENT_PROFILE=managed`
- `docker` 下使用 `docker secret`
- 非 `docker` 下使用 `WALLET_KEK_PATH`
- Owner JWT secret 由部署侧显式提供
- 不允许依赖自动生成的运行时 secret
- 缺失外部 `KEK`、Owner JWT secret 或必要链配置时必须 fail-fast
- 不得因缺失配置自动退回 `quickstart`

### 7.3 开发模式

允许：

- 显式开启自动生成 `KEK`

限制：

- 仅本地开发
- 必须明确标记为非生产模式

### 7.4 `quickstart` 到 `managed` 的接管迁移边界

系统必须允许后续把同一份运行时数据从 `quickstart` 接管到 `managed`。

正式边界：

- 两种档位共用同一镜像与同一数据格式
- 接管迁移必须由运维显式触发，不允许运行中自动切换
- 迁移的核心动作是把现有钱包所依赖的 `KEK` 纳入外部受控提供，或使用新的外部 `KEK` 重新包装 `DEK`
- 切换完成后，系统必须以 `DEPLOYMENT_PROFILE=managed` 重新启动

## 8. 控制权说明

### 8.1 使用 `runtime-auto-file` 时

核心秘钥掌握在：

- 运行时数据目录的拥有者
- 宿主机管理员
- 能读取容器 runtime volume 的人员

这只适用于 `quickstart` 档位。

### 8.2 使用 docker secret 时

核心秘钥掌握在：

- 部署者
- 宿主机管理员

### 8.3 使用受控文件路径时

核心秘钥掌握在：

- 系统管理员
- 具备文件读取权限的部署人员

### 8.4 使用 env 时

核心秘钥掌握在：

- 运行环境管理者
- 可能读取环境变量的相关系统

因此 `env` 不作为正式默认。

## 9. 运行时 secret 目录要求

为支持 quickstart 零应用配置启动，系统必须把自动生成的敏感材料收口到统一目录，例如：

- `/app/runtime-data/secrets/wallet.kek`
- `/app/runtime-data/secrets/owner-jwt.secret`
- `/app/runtime-data/owner-credential.txt`

要求：

- 目录必须位于运行时 volume，不得位于镜像只读层
- quickstart 必须显式挂载外部持久化存储到该目录
- 文件权限必须收敛到仅服务用户可读
- 镜像构建时不得内置任何默认 secret 文件
- `quickstart` 和 `managed` 只共享目录约定，不共享 secret 来源
- 该目录不得被 Agent 可直接读取的运行单元挂载

## 10. 安全禁止事项

明确禁止：

- 私钥明文写入数据库
- 私钥明文写入日志
- `DEK` 明文落库
- `KEK` 落库
- 在调试接口返回私钥或包装材料
- 用 Owner 密码直接加密钱包私钥

最后一条必须明确禁止，因为：

- Owner 密码是登录凭证，不是根密钥
- 凭证轮换不应触发钱包重加密逻辑

## 11. 访问令牌与权限安全

Owner 登录建议：

- 使用 `Bearer JWT`
- 默认短期访问令牌，建议 `1 小时`
- 不提供 refresh token，过期后重新登录

高风险动作要求：

- 已登录
- 二次确认
- 审计记录

## 12. `KEK` 轮换支持边界

本期不提供常规 UI / HTTP API 的 `KEK` 轮换入口，但核心层必须支持：

- 使用当前 `KEK` 解包已存储的 `DEK`
- 使用新 `KEK` 重新包装 `DEK`
- 原子更新钱包中保存的包装后 `DEK`
- 同步更新系统配置快照中的 `kekProvider` 与 `kekReference`
- 记录系统级审计日志

推荐形态：

- 应用层提供独立的 `KEK` 重包装能力
- 部署 / 运维侧通过维护脚本或运维流程触发
- 不作为普通 Owner 能力暴露

## 13. 安全结论

本期安全架构的核心原则是：

`受控部署下私钥由系统使用、根密钥由部署侧掌握；零配置 quickstart 下私钥与根密钥都受运行时数据目录控制。`
