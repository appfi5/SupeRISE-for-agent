...
- `apps/owner-ui/src/localization/`
...
- 风险说明展示
- 登录前后共享的英文 / 中文语言切换能力
- Owner UI 固定文案与产品提示文案双语资源

### 9.4 开发要求

- UI 看板必须完全由原子能力组合而成，不新增后端聚合接口。
- UI 必须按链分组展示资产，但组合逻辑留在前端。
- 当前多语言只覆盖 Owner UI 的固定界面文案和产品提示文案，不扩展到 MCP 或 Owner API 返回模型。
- UI 默认语言必须为英文，并支持切换到中文。
- 当前语言偏好只在本地浏览器侧持久化，不要求 server 保存。
- 登录前后界面必须共享同一语言状态；当本地偏好缺失时回退到英文。
- 固定文案必须通过统一文案资源管理，不允许散落硬编码英文或中文。
- 日期、时间、数字展示与所使用 UI 框架的内置固定文案必须跟随当前语言。
- 固定枚举展示值必须本地化，不直接把原始值当作最终产品文案。
- 用户输入内容、地址、交易哈希、资产符号不进入翻译流程。
...
- Owner 默认进入英文界面，并可切换到中文界面完成主要操作。
- 同一语言下的固定界面文案、产品提示文案、固定枚举展示值与格式化展示结果保持一致。
- 登录前后界面共享同一语言状态。
